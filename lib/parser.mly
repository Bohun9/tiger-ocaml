...
var_compound
  : var DOT ID                  { VField($1, (symbol $3)) }
  | ID L_BRACKET exp R_BRACKET  { VSubscript(VSimple (symbol $1), $3) }
...
exp
  ...
  | INT                                { EInt $1 }
  | STR                                { EString $1 }
  | ID L_PAREN seq(exp, COMMA) R_PAREN { ECall {func = (symbol $1); args = $3} }
  ...
  | ID L_BRACE seq(field_exp, COMMA) R_BRACE { ERecord { typ = (symbol $1); fields = $3 } }
  ...
  | FOR ID ASSIGN exp TO exp DO exp                    { EFor { var = (symbol $2); escape = ref true; lo = $4; hi = $6; body = $8 } }
  | BREAK                                              { EBreak }
  | LET chain(decl) IN exp END                         { ELet { decls = merge_decls $2; body = $4 } }                            
  | ID L_BRACKET exp R_BRACKET OF exp                  { EArray { typ = (symbol $1); size = $3; init = $6 } }

decl
  : FUNCTION ID L_PAREN seq(field_ty, COMMA) R_PAREN EQ exp          { DFunctions [{ fname = (symbol $2); params = $4; result = None; body = $7 }] }
  | FUNCTION ID L_PAREN seq(field_ty, COMMA) R_PAREN COLON ID EQ exp { DFunctions [{ fname = (symbol $2); params = $4; result = Some (symbol $7); body = $9 }] }
  | VAR ID ASSIGN exp                                                { DVar { var = (symbol $2); escape = ref true; annot = None; e = $4 } }
  | VAR ID COLON ID ASSIGN exp                                       { DVar { var = (symbol $2); escape = ref true; annot = Some (symbol $4); e = $6 } }
  | TYPE ID EQ ty                                                    { DTypes [symbol $2, $4] }

ty
  : ID                                   { TName (symbol $1) }
  | L_BRACE seq(field_ty, COMMA) R_BRACE { TRecord $2 }
  | ARRAY OF ID                          { TArray (symbol $3) }

field_exp
  : ID EQ exp { symbol $1, $3 }

field_ty
  : ID COLON ID { { name = (symbol $1); escape = ref true; typ = symbol $3 } }
